...
export function BiomarkersClient({ ehr, wearable }: BiomarkersClientProps) {
  ...
  const cardiovascularMarkers: BiomarkerRowProps[] = [
    ...
    },
    {
      ...
      unit: "mmol/L",
      status: Number(ehr.total_cholesterol_mmol) > 5.0 ? "warning" : "normal",
    ...
    },
    {
      ...
      unit: "mmol/L",
      status: Number(ehr.ldl_mmol) > 3.0 ? "critical" : Number(ehr.ldl_mmol) > 2.6 ? "warning" : "normal",
  ...
  const metabolicMarkers: BiomarkerRowProps[] = [
    {
      ...
      unit: "%",
      status: Number(ehr.hba1c_pct) < 5.7 ? "normal" : Number(ehr.hba1c_pct) < 6.5 ? "warning" : "critical",
      reference: "Ziel: < 5.7%",
    ...
    },
    {
      ...
      unit: "kg/m\u00B2",
      status: Number(ehr.bmi) >= 18.5 && Number(ehr.bmi) <= 24.9 ? "normal" : "warning",
  ...
  const renalInflammatoryMarkers: BiomarkerRowProps[] = [
    {
      name: "eGFR",
      subtitle: "Geschätzte glomeruläre Filtrationsrate",
      value: String(ehr.egfr_ml_min),
      unit: "mL/min",
      status: Number(ehr.egfr_ml_min) >= 90 ? "normal" : Number(ehr.egfr_ml_min) >= 60 ? "warning" : "critical",
    ...
    },
    {
      ...
      unit: "mg/L",
      status: Number(ehr.crp_mg_l) < 1.0 ? "normal" : Number(ehr.crp_mg_l) < 3.0 ? "warning" : "critical",
  ...
  const countByStatus = {
    normal: allMarkers.filter((m) => m.status === "normal").length,
    warning: allMarkers.filter((m) => m.status === "warning").length,
    critical: allMarkers.filter((m) => m.status === "critical").length,
  };
    ...
    <div className="mx-auto max-w-3xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <header className="animate-in mb-8">
        ...
        <p className="mt-2 text-fluid-base text-muted-foreground">
          Klinische Laborwerte und Vitalparameter im Überblick.
        </p>
      ...
      {/* Tabs */}
      <Tabs defaultValue="cardiovascular" className="animate-in stagger-1">
        ...
        <TabsContent value="cardiovascular">
          <BiomarkerSection
            title="Kardiovaskuläre Marker"
            description="Blutdruck- und Lipidprofil zur Einschätzung des Herz-Kreislauf-Risikos."
        ...
        <TabsContent value="renal">
          <BiomarkerSection
            title="Nieren- &amp; Entzündungsmarker"
            description="Nierenfunktion und systemische Entzündungsparameter."
      ...
      <div className="mt-12 border-t border-border pt-6">
        <p className="text-fluid-xs text-muted-foreground leading-relaxed">
          ...
          keine ärztliche Beratung. Alle Biomarker basieren auf den zuletzt
          erfassten Daten und können individuell abweichen.
        </p>
      </div>
    </div>
  );
